...
export function activate(context: vscode.ExtensionContext) {
    ...
    const showMenu = vscode.commands.registerCommand('npm-package-navigator.showPackageMenu', async (explicit?: PackageMatch | string) => {
    ...
    const commands = [
        { command: 'npm-package-navigator.openPackageDirectory', handler: (match: PackageMatch | string) => navigationService.openPackageDirectory(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openPackageJson', handler: (match: PackageMatch | string) => navigationService.openPackageJson(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openReadme', handler: (match: PackageMatch | string) => navigationService.openReadme(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openRepository', handler: (match: PackageMatch | string) => navigationService.openRepository(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openHomepage', handler: (match: PackageMatch | string) => navigationService.openHomepage(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openIssues', handler: (match: PackageMatch | string) => navigationService.openIssues(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openUnpkg', handler: (match: PackageMatch | string) => navigationService.openUnpkg(normalizeMatch(match)) },
        { command: 'npm-package-navigator.openJsDelivr', handler: (match: PackageMatch | string) => navigationService.openJsDelivr(normalizeMatch(match)) }
...
async function resolvePackageMatch(explicit?: PackageMatch | string): Promise<PackageMatch | undefined> {
...
async function showPackageNavigationMenu(match: PackageMatch): Promise<void> {
    ...
    switch (selection.label) {
        ...
        case '$(cloud) unpkg (latest)':
            await vscode.commands.executeCommand('npm-package-navigator.openUnpkg', { name: match.name });
            break;
        ...
        case '$(rocket) jsDelivr (latest)':
            await vscode.commands.executeCommand('npm-package-navigator.openJsDelivr', { name: match.name });
            break;
