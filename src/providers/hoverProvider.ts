...
export class PackageHoverProvider implements vscode.HoverProvider {
  ...
  async provideHover(
    ...
    const packageTitle = `**${match.name}${info?.version ? ` v${info.version}` : ""}**`;
    const npmLink = info?.private ? "" : ` ([npmjs](${npmUrl}))`;
    markdown.appendMarkdown(`${packageTitle}${npmLink}\n\n`);
    ...
    markdown.appendMarkdown(
      `[Open Navigator](command:npm-package-navigator.showPackageMenu?${commandArgs})\n\n`,
    );
